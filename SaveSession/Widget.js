...
define(['dojo/_base/declare',
    ...
    function (declare,
		...
		var dfd= new Deferred();
			  ...
			  for (var i = 0; i < layerSettings.visibleLayers.length; i++) {
			
			
			//console.log(layerSettings.visibleLayers[i]);
                 //data_table += "<tr> <td> " + data.features[i].attributes.source + "</td> <td> " + data.features[i].attributes.average_family_size + "</td><td>" + data.features[i].attributes.source + "</td></tr>";
				 layerVisible.push(layerSettings.visibleLayers[i]);
				  //chartjsLabel.push(data.features[i].attributes.source);
					//chartjsData.push(data.features[i].attributes.average_family_size);				  


             }
						//console.log(layerVisible);
                    layer = new WMSLayer(layerSettings.url, {
            format: "png",
			
			
            visibleLayers: layerVisible
          });
		...
		/* 	loader._waitForLayer(layer).then(function(lyr) {
         ...
         /*  dfd.resolve(lyr);
          ...
          dfd.reject(error);
                    ...
                    //console.log('SaveSession :: addLayerToMap :: created WMSLayer layer = ', layer);
                    break;
            ...
            getSettingsForLayer: function (layer) {
                ...
                case "ArcGISTiledMapServiceLayer":
                    ...
                    break;
				  case "WMSLayer":
                    layerSettings.options = this.getOptionsForWMSLayer(layer);
                    //console.log('SaveSession :: getSettingsForLayer :: added options for WMS layer = ', layerSettings);
                    break;
                default:
...
getOptionsForWMSLayer: function (layer) {
//console.log('4');
                var options = {
                 id: layer.id,
                  format: "png",
                ...
                //console.log('SaveSession :: getOptionsForWMSLayer :: options =  ', options, ' for layer = ', layer.id);
                return options;
            },
            /**
