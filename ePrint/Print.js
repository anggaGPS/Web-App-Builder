    ...
    var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        onLayoutChange: function(newValue) {
            ...
            if (pos > -1) {
                ...
                //console.log(this.customTextElementsTable);
                ...
                on(this.map, "click", lang.hitch(this, function(event) {
					   var objectidt = event.graphic.attributes.objectid;
					   // console.log(event);
						
						
						
					queryTask = new QueryTask(this.defaultAttURL);
					
				//initialize query
					query = new Query();
					query.returnGeometry = true;
					//query.where = "objectId = "+objectidt+"";
					...
					//console.log(event.graphic.attributes.d_nop);
					for (i = 0; i < response.fields.length; i++) { 
						
                            var p = response.fields[i].name;
							//console.log(angga);
							//console.log(response.features["0"].attributes[angga]);
							
							 cte = new Object();
							 cte[p]=event.graphic.attributes[p];
							 //console.log(cte);
							

							if (localStorage.clickcount) {
								localStorage.clickcount = Number(localStorage.clickcount) + 1;
								if (localStorage.clickcount > response.fields.length) {
									 var row = this.customTextElementsTable.deleteRow(0);
                            ...
                            //		tes.splice(0, 2);
                            //customTextElements.clear;
                            //console.log(tes);

                            //console.log(customTextElements);
        ...
        print: function() {
            if (this.printSettingsFormDijit.isValid()) {
                ...
                if (hasLegend &&
                    ...
                    var legends = arcgisUtils.getLegendLayers({
                        map: this.map,
